...
public final class FeatureSwitchImportExport {
    ...
    public static boolean handleActivityResult(
        ...
        if (uri == null) {
            Utils.showToastShort(str("piko_xlite_feature_switch_file_not_selected"));
            return true;
        }
    ...
    private static void importOverrides(Activity activity, Uri uri) {
        try (InputStream input = activity.getContentResolver().openInputStream(uri)) {
            ...
            Utils.showToastShort(str("piko_xlite_feature_switch_import_success"));
        } catch (Exception exception) {
            Logger.printException(() -> "Failed to import X-Lite feature switches", exception);
            Utils.showToastShort(str("piko_xlite_feature_switch_import_failed"));
        }
    }
    ...
    private static void exportOverrides(Activity activity, Uri uri) {
        try (OutputStream output = activity.getContentResolver().openOutputStream(uri)) {
            ...
            Utils.showToastShort(str("piko_xlite_feature_switch_export_success"));
        } catch (Exception exception) {
            Logger.printException(() -> "Failed to export X-Lite feature switches", exception);
            Utils.showToastShort(str("piko_xlite_feature_switch_export_failed"));
        }
    }
    ...
    private static String exportFileName() {
        ...
        return "piko_xlite_feature_switches_" + timestamp + ".json";
    }
}
