        ...
        execute {
            StringFlagCheckMethodFingerprint.apply {
                ...
                method.apply {
                    ...
                    val booleanMethod = methods
                        .firstOrNull {
                            it.returnType == "Z" && it.parameters.size == 3 &&
                                it.parameters[0].type == configValueSourceWrapperClassType
                        } ?: throw PatchException("Failed to find boolean flag check method in ${classDef.type}")

                    booleanMethod.apply {
                        // This is needed in order to find the profile info parameter.
                        val isStaticMethod = AccessFlags.STATIC.isSet(accessFlags)

                        var configSpecifierRegister = parameters.indexOfFirst { it.type == "J" }
                        if (configSpecifierRegister == -1) throw PatchException("Failed to find Long parameter in booleanMethod")
