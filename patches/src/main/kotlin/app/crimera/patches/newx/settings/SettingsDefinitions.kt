...
private val STABLE_ID_PATTERN = Regex("newx\\.[a-z0-9._-]+")
private val OPTION_ID_PATTERN = Regex("[a-zA-Z0-9._-]+")
private val RESOURCE_NAME_PATTERN = Regex("piko_newx_[a-z0-9_]+")
...
internal fun validateSettingsContribution(catalog: SettingsContributionCatalog) {
    require(catalog.categories.isNotEmpty()) { "An NewX settings contribution cannot be empty" }
    ...
    fun validateNode(node: SettingsNodeDefinition) {
        ...
        when (node) {
            is SettingsGroupDefinition -> {
                require(groupIds.add(node.id)) { "Duplicate NewX settings group ID: ${node.id}" }
                node.iconResourceName?.let { iconResourceName ->
                    require(DRAWABLE_RESOURCE_NAME_PATTERN.matches(iconResourceName)) {
                        "Invalid NewX group icon resource: $iconResourceName"
                    }
                }
                require(node.children.isNotEmpty()) { "NewX settings group is empty: ${node.id}" }
                node.children.forEach(::validateNode)
            }

            is SettingItemDefinition -> {
                require(settingIds.add(node.id)) { "Duplicate NewX setting ID: ${node.id}" }
...
private fun validateCommonMetadata(node: SettingsNodeDefinition) {
    require(STABLE_ID_PATTERN.matches(node.id)) { "Invalid NewX settings ID: ${node.id}" }
    require(RESOURCE_NAME_PATTERN.matches(node.titleResourceName)) {
        "Invalid NewX title resource: ${node.titleResourceName}"
    }
    node.summaryResourceName?.let { summary ->
        require(RESOURCE_NAME_PATTERN.matches(summary)) {
            "Invalid NewX summary resource: $summary"
        }
    }
    require(node.order >= 0) { "NewX settings order cannot be negative: ${node.id}" }
}
