...
private object NewXLifecycleAutoRefreshFingerprint : Fingerprint(
...
val disableTimelineRefreshPatch =
    bytecodePatch(
        ...
        execute {
            ...
            val lifecycleMatches = NewXLifecycleAutoRefreshFingerprint.scopedMatchAll()
            if (lifecycleMatches.size != 1) {
                throw PatchException(
                    "Expected one NewX lifecycle auto-refresh coroutine, found " +
            ...
            val requestTypeDescriptor =
                lifecycleMatch.instructionMatches.first()
                    ...
                    ?.definingClass
                    ?: throw PatchException("NewX request type descriptor was not found")
            lifecycleMatch.method.apply {
                val forYouReference =
                    ...
                    } ?: throw PatchException("NewX auto-refresh FOR_YOU check was not found")
                ...
                val timelineResult =
                    instructions
                        ...
                        "NewX auto-refresh timeline type result was not found",
                    )
                ...
                val timelineGetterReference =
                    timelineGetterCall.getReference<MethodReference>()
                        ?: throw PatchException("NewX timeline type getter was not found")
                val timelineGetterInstruction =
                    timelineGetterCall as? FiveRegisterInstruction
                        ?: throw PatchException("NewX timeline type getter does not use explicit registers")
                if (timelineGetterInstruction.registerCount != 1) {
                    throw PatchException("Unexpected NewX timeline type getter register layout")
                }
                ...
                if (autoRefreshLoads.size != 3) {
                    throw PatchException(
                        "Expected three NewX AUTO_REFRESH request loads, found " +
                ...
                autoRefreshLoads.asReversed().forEachIndexed { reverseIndex, autoRefreshLoad ->
                    ...
                    val requestCall =
                        instructions
                            ...
                            ?: throw PatchException(
                                "NewX auto-refresh repository call was not found",
                            )
                    ...
                    addInstructionsWithLabels(
                        read.nextIndex,
                        """
                            if-eqz v$settingRegister, :piko_newx_refresh_lifecycle_continue_$labelSuffix
                            ...
                            if-ne v$timelineRegister, v$settingRegister, :piko_newx_refresh_lifecycle_check_following_$labelSuffix
                            sget-object v$requestTypeRegister, $requestTypeDescriptor->VIEWPORT_AWARE_AUTO_REFRESH:$requestTypeDescriptor
                            goto :piko_newx_refresh_lifecycle_continue_$labelSuffix
                            :piko_newx_refresh_lifecycle_check_following_$labelSuffix
                            sget-object v$settingRegister, $timelineTypeDescriptor->FOLLOWING:$timelineTypeDescriptor
                            if-ne v$timelineRegister, v$settingRegister, :piko_newx_refresh_lifecycle_continue_$labelSuffix
                            sget-object v$requestTypeRegister, $requestTypeDescriptor->VIEWPORT_AWARE_AUTO_REFRESH:$requestTypeDescriptor
                        """.trimIndent(),
                        ExternalLabel(
                            "piko_newx_refresh_lifecycle_continue_$labelSuffix",
