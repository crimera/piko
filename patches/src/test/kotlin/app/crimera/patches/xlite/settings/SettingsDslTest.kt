...
class SettingsDslTest {
    ...
    fun `advanced DSL returns typed feature settings and indexes its catalog`() {
        ...
        try {
            ...
            val patch =
                bytecodePatch(name = "Advanced X-Lite settings test", default = false) {
                    settings =
                        xLiteSettings {
                                    ...
                                    FeatureSettings(
                                        enabled =
                                            toggle(
                                                ...
                                                strings =
                                                    settingStrings("piko_xlite_post_filtering_enabled"),
                                                order = 100,
                                                defaultValue = false,
                                            ),
                                        blockedWords =
                                            input(
                                                ...
                                                strings =
                                                    settingStrings(
                                                        "piko_xlite_post_filtering_blocked_words",
                                                    ),
                                                order = 200,
                                                defaultValue = "",
                                        ...
                                        actions =
                                            multiChoice(
                                                id = "xlite.content.post_filtering.actions",
                                                strings =
                                                    settingStrings("piko_xlite_post_filtering_actions"),
                                                order = 300,
                                                ...
                                                options =
                                                    listOf(
                                                        choice(
                                                            "reply",
                                                            "piko_xlite_post_filtering_reply_title",
                                                        ),
                                                    ),
                                            ),
                                        clear =
                                            action(
                                                id = "xlite.content.post_filtering.clear",
                                                strings =
                                                    settingStrings(
                                                        "piko_xlite_post_filtering_clear",
                                                        summary = false,
                                                    ),
                                                order = 400,
                                                handlerClassDescriptor =
                                                    "Lapp/morphe/extension/xlite/content/ClearFiltersAction;",
                                            ),
                                    )
                                }
                            }
            ...
            assertEquals("xlite.content.post_filtering.enabled", settings.enabled.id)
            assertEquals(InputKind.MULTILINE, settings.blockedWords.inputKind)
            assertEquals(setOf("reply"), settings.actions.defaultValue)
            assertEquals("xlite.content.post_filtering.clear", settings.clear.id)
            ...
            assertEquals(
                listOf(
                    "xlite.content.post_filtering.enabled",
                    "xlite.content.post_filtering.blocked_words",
                    "xlite.content.post_filtering.actions",
                    "xlite.content.post_filtering.clear",
                ),
