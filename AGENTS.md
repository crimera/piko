...
platform APIs introduced after API 29, including Java 21 `List` methods such as `removeLast()`
and `removeFirst()`. For stack-style list access, use the API-safe form
`list.removeAt(list.lastIndex)` (or `list.remove(list.size - 1)`) and inspect generated bytecode
when using newer Kotlin collection APIs.

### Compose and dynamic fingerprint resilience

Jetpack Compose and R8 introduce continuous bytecode churn across app versions:
- **Compiler lowering & synthetic parameters**: The Compose compiler rewrites `@Composable`
  signatures, injecting synthetic `$composer` parameters, bitmask change flags (`$changed`,
  `$changed1`), default flags (`$default`), and unboxing inline value classes (e.g. `Color` to `J`).
- **Signature drift**: Adding optional parameters (e.g. `Modifier`, test tags, lambdas) alters
  lowered method arity and parameter order across alpha and production releases.
- **Avoid rigid parameter lists**: Never match a composable function by an exact, fixed-length
  parameter list (e.g. hardcoding 10 specific descriptors). Doing so causes cascading patch
  failures whenever upstream adds a parameter.
- **Match semantic invariants**: Match on core domain contracts (e.g. package scope, required
